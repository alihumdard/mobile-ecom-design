...
const Similler = () => {
  ...
  return (
    <div>
      {/* Header Section */}
      <div className="bg-slate-100 px-4 sm:px-6 md:px-10 lg:px-20 xl:px-40 py-8 flex flex-col md:flex-row justify-between gap-4">
        <div>
          <h1 className="text-lg font-semibold pb-2">Similar Mobiles to ZTE Nubia Focus 5g</h1>
          <p>These are some mobiles you can buy similar to ZTE Nubia Focus 5g. I think you’ll like them, too.</p>
        </div>
        <div className="flex justify-start md:justify-end">
          <button className="px-6 py-3 bg-primary text-white rounded-md text-sm">Compare</button>
        </div>
      </div>
      ...
      <div className="bg-slate-100">
        <Section title="Shop More Categories" className="pt-10">
          <div className="bg-[#f9dcff] px-4 sm:px-6 md:px-10 lg:px-20 xl:px-40 flex flex-wrap gap-5 justify-center pt-6 pb-10">
            {[
              ...
              >
                <img src={item.image} className="w-20 h-20 object-contain mb-2" alt={item.label} />
                <div className="bg-[#8804a6] w-full py-4 rounded-b-2xl text-center">
        ...
        {/* Popular Section */}
        <Section title="Popular Among Customers">
          <ProductGrid products={products} />
        </Section>

        {/* Top Seller Section */}
        <Section title="Priceoye’s Top Seller">
          <ProductGrid products={products} />
        </Section>

        {/* Shop by Price */}
        <Section title="Shop by Price">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 bg-[#fdc9da] py-10 px-4 sm:px-6 md:px-10 lg:px-20 xl:px-40">
...
const Section = ({ title, className = "", children }) => (
  <div className={`px-4 sm:px-6 md:px-10 lg:px-20 xl:px-40 py-6 ${className}`}>
    ...
    {products.map((product, i) => (
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <del className="text-sm text-gray-400">Rs {product.oldPrice.toLocaleString()}</del>
