...
const Installment = () => {
  const [openAlfalah, setOpenAlfalah] = useState(false);
  const [openFaysal, setOpenFaysal] = useState(false);
  const [openHBL, setOpenHBL] = useState(false);
  const [openJS, setOpenJS] = useState(false);
  const [openMCB, setOpenMCB] = useState(false);
  const [openUBL, setOpenUBL] = useState(false);

  const bankData = (id) => banks.find((b) => b.id === id).data;
  const bankImage = (id) => banks.find((b) => b.id === id).image;
  const bankName = (id) => banks.find((b) => b.id === id).name;

  const renderDropdown = (isOpen, toggleOpen, id) => (
    <>
      <div
        onClick={toggleOpen}
      ...
      >
        <div className="flex items-center gap-3">
          <img src={bankImage(id)} alt={bankName(id)} className="w-6 h-6" />
          <span className="font-medium text-gray-800">{bankName(id)}</span>
        </div>
      ...
      {isOpen && (
        <div className="mt-4">
          ...
          <ul className="mt-4 text-xs text-gray-700 space-y-1 list-decimal list-inside">
            <li>
              One Time Bank Fee: Flat 2.50% or Rs.1,000 and FED will be charged
              by the bank.
            </li>
            <li>Processing Fee: 2.5% will be charged.</li>
            <li>Please ensure Name and CNIC are of the cardholder.</li>
            <li>SBS Plan will be processed as per assigned credit limit.</li>
          </ul>
        </div>
      )}
    </>
  ...
  return (
    <>
      <div>
        <div>
          <h1 className="px-40 py-10 text-lg bg-slate-100">
            Installment Plans
          </h1>
        </div>

        <div className="mx-auto px-40 pt-6 pb-2">
          <p className="text-xs text-gray-600 mb-7">
            *Only Credit Card holders are eligible for installment plan
          </p>

          {/* Individual Bank Dropdowns */}
          {renderDropdown(
            openAlfalah,
            () => setOpenAlfalah(!openAlfalah),
            "alfalah"
          )}
          {renderDropdown(
            openFaysal,
            () => setOpenFaysal(!openFaysal),
            "faysal"
          )}
          {renderDropdown(openHBL, () => setOpenHBL(!openHBL), "hbl")}
          {renderDropdown(openJS, () => setOpenJS(!openJS), "js")}
          {renderDropdown(openMCB, () => setOpenMCB(!openMCB), "mcb")}
          {renderDropdown(openUBL, () => setOpenUBL(!openUBL), "ubl")}
        </div>
      </div>

      <div className="bg-slate-100 px-40 py-8 mt-5 flex justify-between">
        <div>
          <h1 className="text-lg font-semibold pb-2">Upgrade to Premium</h1>
        </div>
        <div>
      ...
      {/* shop by brand */}
      <div>
        <h1 className="text-lg font-semibold bg-slate-100 px-40 py-8 mt-5">
          Shop By Brand
        </h1>
      </div>
